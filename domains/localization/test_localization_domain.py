"""domains/localization/test_localization_domain.py"""
import inspect
def test_manifest_valido():
    from domains.localization.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "localization"; assert len(MANIFEST.ops) == 3
def test_ops():
    from domains.localization.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.localization import handlers
