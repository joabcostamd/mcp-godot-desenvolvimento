"""domains/audio/test_audio_domain.py — 8 testes (F5.17)."""
import pytest, inspect

def test_manifest_valido():
    from domains.audio.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "audio"; assert len(MANIFEST.ops) == 4

def test_todas_ops_tem_docstring():
    from domains.audio.manifest import MANIFEST
...
def test_handlers_keyword_only():
    from domains.audio import handlers
