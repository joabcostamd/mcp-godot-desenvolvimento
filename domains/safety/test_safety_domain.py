"""domains/safety/test_safety_domain.py — F5.24."""
import inspect
def test_manifest_valido():
    from domains.safety.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "safety"; assert len(MANIFEST.ops) == 5
def test_ops():
    from domains.safety.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.safety import handlers
