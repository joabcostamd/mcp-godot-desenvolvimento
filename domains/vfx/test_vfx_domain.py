"""domains/vfx/test_vfx_domain.py — 8 testes obrigatórios do domínio vfx (F5.6)."""
...
def test_handlers_keyword_only():
    ...
    from domains.vfx import handlers
    for name in handlers.__all__:
        fn = getattr(handlers, name)

        # Deve rejeitar chamada posicional
        import inspect
        ...
        # Todos os parâmetros devem ser keyword-only ou ter default
        for p in params:
            assert p.kind in (
