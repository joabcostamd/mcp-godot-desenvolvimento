"""domains/raycast/test_raycast_domain.py"""
import inspect
def test_manifest_valido():
    from domains.raycast.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "raycast"; assert len(MANIFEST.ops) == 2
def test_ops():
    from domains.raycast.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.raycast import handlers
