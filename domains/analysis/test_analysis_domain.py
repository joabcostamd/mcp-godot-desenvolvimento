"""domains/analysis/test_analysis_domain.py — F5.25."""
import inspect
def test_manifest_valido():
    from domains.analysis.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "analysis"; assert len(MANIFEST.ops) == 7
def test_ops():
    from domains.analysis.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.analysis import handlers
