"""domains/file/test_file_domain.py"""
import inspect
def test_manifest_valido():
    from domains.file.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "file"; assert len(MANIFEST.ops) == 3
def test_ops():
    from domains.file.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.file import handlers
