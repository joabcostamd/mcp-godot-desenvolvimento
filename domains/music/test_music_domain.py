"""domains/music/test_music_domain.py"""
import inspect
def test_manifest_valido():
    from domains.music.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "music"; assert len(MANIFEST.ops) == 4
def test_ops():
    from domains.music.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.music import handlers
