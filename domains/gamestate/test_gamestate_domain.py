"""domains/gamestate/test_gamestate_domain.py — F5.22."""
import inspect
def test_manifest_valido():
    from domains.gamestate.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "gamestate"; assert len(MANIFEST.ops) == 4
def test_ops():
    from domains.gamestate.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.gamestate import handlers
