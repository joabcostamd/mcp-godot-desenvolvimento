"""domains/script/test_script_domain.py"""
import inspect
def test_manifest_valido():
    from domains.script.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "script"; assert len(MANIFEST.ops) == 6
def test_ops():
    from domains.script.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.script import handlers
