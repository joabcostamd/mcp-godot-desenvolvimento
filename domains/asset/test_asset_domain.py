"""domains/asset/test_asset_domain.py"""
import inspect
def test_manifest_valido():
    from domains.asset.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "asset"; assert len(MANIFEST.ops) == 11
def test_ops():
    from domains.asset.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.asset import handlers
