"""domains/config/test_config_domain.py — F5.23."""
import inspect
def test_manifest_valido():
    from domains.config.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "config"; assert len(MANIFEST.ops) == 2
def test_ops():
    from domains.config.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.config import handlers
