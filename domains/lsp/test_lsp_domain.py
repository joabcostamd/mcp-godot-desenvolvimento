"""domains/lsp/test_lsp_domain.py — 8 testes obrigatórios do domínio lsp (F5.10)."""
...
def test_manifest_valido():
    from domains.lsp.manifest import MANIFEST
    from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest)
    assert MANIFEST.domain == "lsp"
    assert MANIFEST.tool_name == "lsp_manage"
...
def test_handlers_keyword_only():
    from domains.lsp import handlers
