"""domains/test/test_test_domain.py — 8 testes obrigatórios do domínio test (F5.14)."""
import pytest, inspect

def test_manifest_valido():
    from domains.test.manifest import MANIFEST
    from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest)
    assert MANIFEST.domain == "test"
    assert MANIFEST.tool_name == "test_manage"
...
def test_handlers_keyword_only():
    from domains.test import handlers
