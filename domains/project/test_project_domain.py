"""domains/project/test_project_domain.py"""
import inspect
def test_manifest_valido():
    from domains.project.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "project"; assert len(MANIFEST.ops) == 5
def test_ops():
    from domains.project.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.project import handlers
