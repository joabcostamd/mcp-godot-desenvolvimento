"""domains/godot/test_godot_domain.py — 8 testes obrigatórios do domínio godot (F5.11)."""
import pytest, inspect

def test_manifest_valido():
    from domains.godot.manifest import MANIFEST
    from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest)
    assert MANIFEST.domain == "godot"
    assert MANIFEST.tool_name == "godot_manage"
...
def test_handlers_keyword_only():
    from domains.godot import handlers
