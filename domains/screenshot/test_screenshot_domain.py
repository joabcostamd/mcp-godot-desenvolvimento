"""domains/screenshot/test_screenshot_domain.py — F6.4."""
import inspect
def test_manifest_valido():
    from domains.screenshot.manifest import MANIFEST; from registry.types import DomainManifest
    assert isinstance(MANIFEST, DomainManifest); assert MANIFEST.domain == "screenshot"; assert len(MANIFEST.ops) == 3
def test_ops():
    from domains.screenshot.manifest import MANIFEST
...
def test_handlers_kw():
    from domains.screenshot import handlers
