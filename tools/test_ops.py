...
def _invoke_tool_synthetic(tool: str, args: dict) -> dict:
    ...
    if tool == "create_entity":
        return {"status": "success", "entity_type": args.get("entity_type", "CharacterBody2D"), "entity_name": args.get("name", "NewEntity"), "node_path": "/root/Game/NewEntity", "message": "Entidade criada (simulada)."}
    if tool == "create_entities":
        return {"status": "success", "count": args.get("count", 1), "entities": [{"name": "Entity_1", "type": "Node2D"}], "message": "Entidades criadas (simuladas)."}
...
def test_coverage_report(args=None):
    ...
    for tool in tools:
        n=tool.name
        # Ordem corrigida: handlers sinteticos ANTES dos skip lists
        # Isso garante que tools com handler sao contadas como cobertas
        ...
        # So cai em skip se NAO tem handler sintetico
...
def run_canary_queries(args=None):
    ...
    try: data=json.loads(cp.read_text(encoding="utf-8")); canaries=data.get("canaries",[])
