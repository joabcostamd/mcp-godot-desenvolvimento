## Teste de composicao: inventory + state_machine
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: inventory
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/inventory/inventory.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/inventory/inventory.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("inventory foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("item_added")).override_failure_message("Sinal 'item_added' nao encontrado em inventory")

	# Sinal 'item_removed': verifica conectividade
	assert_bool(_behavior_a.has_signal("item_removed")).override_failure_message("Sinal 'item_removed' nao encontrado em inventory")

	# Sinal 'inventory_full': verifica conectividade
	assert_bool(_behavior_a.has_signal("inventory_full")).override_failure_message("Sinal 'inventory_full' nao encontrado em inventory")
