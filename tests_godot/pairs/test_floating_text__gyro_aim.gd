## Teste de composicao: floating_text + gyro_aim
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: floating_text
## @behavior_b: gyro_aim

extends GdUnitTestSuite


var _behavior_a: Node
var _behavior_b: Node


func after() -> void:
	if is_instance_valid(_behavior_a):
		_behavior_a.queue_free()
	if is_instance_valid(_behavior_b):
		_behavior_b.queue_free()


func test_coexistencia() -> void:
	var res_a := load("res://behaviors/floating_text/floating_text.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/floating_text/floating_text.tscn")
	_behavior_a = res_a.instantiate()
	assert_object(_behavior_a).is_not_null()

	var res_b := load("res://behaviors/gyro_aim/gyro_aim.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/gyro_aim/gyro_aim.tscn")
	_behavior_b = res_b.instantiate()
	assert_object(_behavior_b).is_not_null()

	add_child(_behavior_a)
	add_child(_behavior_b)

	await get_tree().create_timer(2.0).timeout

	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("floating_text foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("gyro_aim foi liberado durante o teste")
	# ── Verifica que floating_text ainda consegue emitir sinais ──
	# Sinal 'text_shown': verifica conectividade
	assert_bool(_behavior_a.has_signal("text_shown")).override_failure_message("Sinal 'text_shown' nao encontrado em floating_text")
	# ── Verifica que gyro_aim ainda consegue emitir sinais ──
	# Sinal 'gyro_active': verifica conectividade
	assert_bool(_behavior_b.has_signal("gyro_active")).override_failure_message("Sinal 'gyro_active' nao encontrado em gyro_aim")

