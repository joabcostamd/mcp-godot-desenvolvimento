## Teste de composicao: double_jump + player_controller
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: double_jump
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/double_jump/double_jump.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/double_jump/double_jump.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("double_jump foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("player_controller foi liberado durante o teste")
	# ── Verifica que double_jump ainda consegue emitir sinais ──
	# Sinal 'jumped': verifica conectividade
	assert_bool(_behavior_a.has_signal("jumped")).override_failure_message("Sinal 'jumped' nao encontrado em double_jump")

	# Sinal 'jumps_exhausted': verifica conectividade
	assert_bool(_behavior_a.has_signal("jumps_exhausted")).override_failure_message("Sinal 'jumps_exhausted' nao encontrado em double_jump")
