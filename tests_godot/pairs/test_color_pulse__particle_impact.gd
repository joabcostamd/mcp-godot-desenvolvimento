## Teste de composicao: color_pulse + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: color_pulse
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/color_pulse/color_pulse.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/color_pulse/color_pulse.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("color_pulse foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("particle_impact foi liberado durante o teste")
	# ── Verifica que color_pulse ainda consegue emitir sinais ──
	# Sinal 'pulsing': verifica conectividade
	assert_bool(_behavior_a.has_signal("pulsing")).override_failure_message("Sinal 'pulsing' nao encontrado em color_pulse")
