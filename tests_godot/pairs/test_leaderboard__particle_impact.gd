## Teste de composicao: leaderboard + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: leaderboard
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/leaderboard/leaderboard.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/leaderboard/leaderboard.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("leaderboard foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("score_submitted")).override_failure_message("Sinal 'score_submitted' nao encontrado em leaderboard")
