## Teste de composicao: steam_input + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: steam_input
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/steam_input/steam_input.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/steam_input/steam_input.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("steam_input foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("action_set_changed")).override_failure_message("Sinal 'action_set_changed' nao encontrado em steam_input")
