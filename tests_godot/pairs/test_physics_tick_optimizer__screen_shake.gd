## Teste de composicao: physics_tick_optimizer + screen_shake
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: physics_tick_optimizer
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/physics_tick_optimizer/physics_tick_optimizer.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/physics_tick_optimizer/physics_tick_optimizer.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("physics_tick_optimizer foi liberado durante o teste")
