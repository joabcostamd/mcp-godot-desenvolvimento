## Teste de composicao: text_size + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: text_size
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/text_size/text_size.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/text_size/text_size.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("text_size foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("size_changed")).override_failure_message("Sinal 'size_changed' nao encontrado em text_size")
