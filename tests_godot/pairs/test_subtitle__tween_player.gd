## Teste de composicao: subtitle + tween_player
##
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/subtitle/subtitle.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/subtitle/subtitle.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("subtitle foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("shown")).override_failure_message("Sinal 'shown' nao encontrado em subtitle")

	# Sinal 'hidden': verifica conectividade
	assert_bool(_behavior_a.has_signal("hidden")).override_failure_message("Sinal 'hidden' nao encontrado em subtitle")
