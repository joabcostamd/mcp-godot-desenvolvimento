## Teste de composicao: screen_flash + screen_shake
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: screen_flash
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/screen_flash/screen_flash.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/screen_flash/screen_flash.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("screen_flash foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("flashed")).override_failure_message("Sinal 'flashed' nao encontrado em screen_flash")
