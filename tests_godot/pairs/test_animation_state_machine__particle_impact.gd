## Teste de composicao: animation_state_machine + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: animation_state_machine
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/animation_state_machine/animation_state_machine.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/animation_state_machine/animation_state_machine.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("animation_state_machine foi liberado durante o teste")
