## Teste de composicao: player_vehicle + status_effect
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: player_vehicle
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/player_vehicle/player_vehicle.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/player_vehicle/player_vehicle.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("player_vehicle foi liberado durante o teste")
