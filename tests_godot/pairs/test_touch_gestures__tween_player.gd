## Teste de composicao: touch_gestures + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: touch_gestures
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/touch_gestures/touch_gestures.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/touch_gestures/touch_gestures.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("touch_gestures foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("swiped")).override_failure_message("Sinal 'swiped' nao encontrado em touch_gestures")

	# Sinal 'pinched': verifica conectividade
	assert_bool(_behavior_a.has_signal("pinched")).override_failure_message("Sinal 'pinched' nao encontrado em touch_gestures")

	# Sinal 'long_pressed': verifica conectividade
	assert_bool(_behavior_a.has_signal("long_pressed")).override_failure_message("Sinal 'long_pressed' nao encontrado em touch_gestures")

	# Sinal 'double_tapped': verifica conectividade
	assert_bool(_behavior_a.has_signal("double_tapped")).override_failure_message("Sinal 'double_tapped' nao encontrado em touch_gestures")
