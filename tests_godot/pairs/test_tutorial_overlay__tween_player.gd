## Teste de composicao: tutorial_overlay + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: tutorial_overlay
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/tutorial_overlay/tutorial_overlay.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/tutorial_overlay/tutorial_overlay.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("tutorial_overlay foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("step_completed")).override_failure_message("Sinal 'step_completed' nao encontrado em tutorial_overlay")

	# Sinal 'tutorial_finished': verifica conectividade
	assert_bool(_behavior_a.has_signal("tutorial_finished")).override_failure_message("Sinal 'tutorial_finished' nao encontrado em tutorial_overlay")
