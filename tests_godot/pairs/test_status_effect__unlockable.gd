## Teste de composicao: status_effect + unlockable
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: status_effect
## @behavior_b: unlockable
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/unlockable/unlockable.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/unlockable/unlockable.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("unlockable foi liberado durante o teste")
	...
	# ── Verifica que unlockable ainda consegue emitir sinais ──
	# Sinal 'unlocked': verifica conectividade
	assert_bool(_behavior_b.has_signal("unlocked")).override_failure_message("Sinal 'unlocked' nao encontrado em unlockable")
