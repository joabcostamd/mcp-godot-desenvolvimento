## Teste de composicao: pause_menu + screen_shake
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: pause_menu
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/pause_menu/pause_menu.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/pause_menu/pause_menu.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("pause_menu foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("paused")).override_failure_message("Sinal 'paused' nao encontrado em pause_menu")

	# Sinal 'resumed': verifica conectividade
	assert_bool(_behavior_a.has_signal("resumed")).override_failure_message("Sinal 'resumed' nao encontrado em pause_menu")
