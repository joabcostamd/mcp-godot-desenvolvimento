## Teste de composicao: input_buffer + state_machine
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: input_buffer
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/input_buffer/input_buffer.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/input_buffer/input_buffer.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("input_buffer foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("buffered")).override_failure_message("Sinal 'buffered' nao encontrado em input_buffer")

	# Sinal 'executed': verifica conectividade
	assert_bool(_behavior_a.has_signal("executed")).override_failure_message("Sinal 'executed' nao encontrado em input_buffer")
