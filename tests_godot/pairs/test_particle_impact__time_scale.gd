## Teste de composicao: particle_impact + time_scale
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: particle_impact
## @behavior_b: time_scale
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/time_scale/time_scale.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/time_scale/time_scale.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("time_scale foi liberado durante o teste")
	...
	# ── Verifica que time_scale ainda consegue emitir sinais ──
	# Sinal 'scale_changed': verifica conectividade
	assert_bool(_behavior_b.has_signal("scale_changed")).override_failure_message("Sinal 'scale_changed' nao encontrado em time_scale")
