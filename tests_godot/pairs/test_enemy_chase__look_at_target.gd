## Teste de composicao: enemy_chase + look_at_target
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: enemy_chase
## @behavior_b: look_at_target
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/look_at_target/look_at_target.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/look_at_target/look_at_target.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("look_at_target foi liberado durante o teste")
	...
	# Sinal 'target_acquired': verifica conectividade
	assert_bool(_behavior_b.has_signal("target_acquired")).override_failure_message("Sinal 'target_acquired' nao encontrado em look_at_target")

	# Sinal 'target_lost': verifica conectividade
	assert_bool(_behavior_b.has_signal("target_lost")).override_failure_message("Sinal 'target_lost' nao encontrado em look_at_target")
