## Teste de composicao: profile_manager + screen_shake
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: profile_manager
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/profile_manager/profile_manager.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/profile_manager/profile_manager.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("profile_manager foi liberado durante o teste")
