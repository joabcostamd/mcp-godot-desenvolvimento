## Teste de composicao: event_bus + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: event_bus
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/event_bus/event_bus.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/event_bus/event_bus.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("event_bus foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("particle_impact foi liberado durante o teste")
	# ── Verifica que event_bus ainda consegue emitir sinais ──
	# Sinal 'event_fired': verifica conectividade
	assert_bool(_behavior_a.has_signal("event_fired")).override_failure_message("Sinal 'event_fired' nao encontrado em event_bus")
