## Teste de composicao: skill_tree + status_effect
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: skill_tree
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/skill_tree/skill_tree.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/skill_tree/skill_tree.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("skill_tree foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("node_unlocked")).override_failure_message("Sinal 'node_unlocked' nao encontrado em skill_tree")

	# Sinal 'tree_reset': verifica conectividade
	assert_bool(_behavior_a.has_signal("tree_reset")).override_failure_message("Sinal 'tree_reset' nao encontrado em skill_tree")

	# Sinal 'points_changed': verifica conectividade
	assert_bool(_behavior_a.has_signal("points_changed")).override_failure_message("Sinal 'points_changed' nao encontrado em skill_tree")
