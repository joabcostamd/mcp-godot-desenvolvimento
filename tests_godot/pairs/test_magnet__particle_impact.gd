## Teste de composicao: magnet + particle_impact
##
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/magnet/magnet.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/magnet/magnet.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("magnet foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("particle_impact foi liberado durante o teste")
	# ── Verifica que magnet ainda consegue emitir sinais ──
	# Sinal 'attracted': verifica conectividade
	assert_bool(_behavior_a.has_signal("attracted")).override_failure_message("Sinal 'attracted' nao encontrado em magnet")
