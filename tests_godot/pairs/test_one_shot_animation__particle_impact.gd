## Teste de composicao: one_shot_animation + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: one_shot_animation
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/one_shot_animation/one_shot_animation.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/one_shot_animation/one_shot_animation.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("one_shot_animation foi liberado durante o teste")
