## Teste de composicao: spring_joint + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: spring_joint
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/spring_joint/spring_joint.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/spring_joint/spring_joint.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("spring_joint foi liberado durante o teste")
