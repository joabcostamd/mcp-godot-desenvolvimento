## Teste de composicao: particle_impact + screen_flash
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: particle_impact
## @behavior_b: screen_flash
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/screen_flash/screen_flash.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/screen_flash/screen_flash.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("screen_flash foi liberado durante o teste")
	...
	# ── Verifica que screen_flash ainda consegue emitir sinais ──
	# Sinal 'flashed': verifica conectividade
	assert_bool(_behavior_b.has_signal("flashed")).override_failure_message("Sinal 'flashed' nao encontrado em screen_flash")
