## Teste de composicao: state_machine + upgrade
##
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/upgrade/upgrade.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/upgrade/upgrade.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("upgrade foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("choices_ready")).override_failure_message("Sinal 'choices_ready' nao encontrado em upgrade")

	# Sinal 'upgrade_applied': verifica conectividade
	assert_bool(_behavior_b.has_signal("upgrade_applied")).override_failure_message("Sinal 'upgrade_applied' nao encontrado em upgrade")
