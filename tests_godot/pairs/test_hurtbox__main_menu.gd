## Teste de composicao: hurtbox + main_menu
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: hurtbox
## @behavior_b: main_menu
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/main_menu/main_menu.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/main_menu/main_menu.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("main_menu foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("play_pressed")).override_failure_message("Sinal 'play_pressed' nao encontrado em main_menu")

	# Sinal 'settings_pressed': verifica conectividade
	assert_bool(_behavior_b.has_signal("settings_pressed")).override_failure_message("Sinal 'settings_pressed' nao encontrado em main_menu")

	# Sinal 'quit_pressed': verifica conectividade
	assert_bool(_behavior_b.has_signal("quit_pressed")).override_failure_message("Sinal 'quit_pressed' nao encontrado em main_menu")
