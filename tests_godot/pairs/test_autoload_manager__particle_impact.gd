## Teste de composicao: autoload_manager + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: autoload_manager
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/autoload_manager/autoload_manager.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/autoload_manager/autoload_manager.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("autoload_manager foi liberado durante o teste")
