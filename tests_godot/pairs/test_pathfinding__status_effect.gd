## Teste de composicao: pathfinding + status_effect
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: pathfinding
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/pathfinding/pathfinding.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/pathfinding/pathfinding.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("pathfinding foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("path_found")).override_failure_message("Sinal 'path_found' nao encontrado em pathfinding")

	# Sinal 'path_lost': verifica conectividade
	assert_bool(_behavior_a.has_signal("path_lost")).override_failure_message("Sinal 'path_lost' nao encontrado em pathfinding")

	# Sinal 'arrived': verifica conectividade
	assert_bool(_behavior_a.has_signal("arrived")).override_failure_message("Sinal 'arrived' nao encontrado em pathfinding")
