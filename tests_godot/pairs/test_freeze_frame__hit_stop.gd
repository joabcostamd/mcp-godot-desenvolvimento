## Teste de composicao: freeze_frame + hit_stop
##
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/hit_stop/hit_stop.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/hit_stop/hit_stop.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("hit_stop foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("hit_stopped")).override_failure_message("Sinal 'hit_stopped' nao encontrado em hit_stop")

	# Sinal 'resumed': verifica conectividade
	assert_bool(_behavior_b.has_signal("resumed")).override_failure_message("Sinal 'resumed' nao encontrado em hit_stop")
