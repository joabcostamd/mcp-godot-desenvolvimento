## Teste de composicao: lerp_smooth + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: lerp_smooth
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/lerp_smooth/lerp_smooth.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/lerp_smooth/lerp_smooth.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("lerp_smooth foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("particle_impact foi liberado durante o teste")
	# ── Verifica que lerp_smooth ainda consegue emitir sinais ──
	# Sinal 'completed': verifica conectividade
	assert_bool(_behavior_a.has_signal("completed")).override_failure_message("Sinal 'completed' nao encontrado em lerp_smooth")
