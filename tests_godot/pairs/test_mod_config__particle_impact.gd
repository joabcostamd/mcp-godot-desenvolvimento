## Teste de composicao: mod_config + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: mod_config
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/mod_config/mod_config.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/mod_config/mod_config.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("mod_config foi liberado durante o teste")
