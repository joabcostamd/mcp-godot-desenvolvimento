## Teste de composicao: resource_factory + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: resource_factory
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/resource_factory/resource_factory.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/resource_factory/resource_factory.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("resource_factory foi liberado durante o teste")
