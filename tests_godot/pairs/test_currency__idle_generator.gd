## Teste de composicao: currency + idle_generator
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: currency
## @behavior_b: idle_generator
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/idle_generator/idle_generator.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/idle_generator/idle_generator.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("idle_generator foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("generated")).override_failure_message("Sinal 'generated' nao encontrado em idle_generator")

	# Sinal 'storage_full': verifica conectividade
	assert_bool(_behavior_b.has_signal("storage_full")).override_failure_message("Sinal 'storage_full' nao encontrado em idle_generator")

	# Sinal 'upgraded': verifica conectividade
	assert_bool(_behavior_b.has_signal("upgraded")).override_failure_message("Sinal 'upgraded' nao encontrado em idle_generator")
