## Teste de composicao: health + high_contrast
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: health
## @behavior_b: high_contrast
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/high_contrast/high_contrast.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/high_contrast/high_contrast.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("high_contrast foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("contrast_changed")).override_failure_message("Sinal 'contrast_changed' nao encontrado em high_contrast")
