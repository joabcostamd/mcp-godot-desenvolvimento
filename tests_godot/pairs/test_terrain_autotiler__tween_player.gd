## Teste de composicao: terrain_autotiler + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: terrain_autotiler
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/terrain_autotiler/terrain_autotiler.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/terrain_autotiler/terrain_autotiler.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("terrain_autotiler foi liberado durante o teste")
