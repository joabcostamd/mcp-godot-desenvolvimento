## Teste de composicao: performance_monitor + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: performance_monitor
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/performance_monitor/performance_monitor.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/performance_monitor/performance_monitor.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("performance_monitor foi liberado durante o teste")
