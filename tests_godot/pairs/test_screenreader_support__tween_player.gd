## Teste de composicao: screenreader_support + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: screenreader_support
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/screenreader_support/screenreader_support.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/screenreader_support/screenreader_support.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("screenreader_support foi liberado durante o teste")
