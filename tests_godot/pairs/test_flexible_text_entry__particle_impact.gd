## Teste de composicao: flexible_text_entry + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: flexible_text_entry
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/flexible_text_entry/flexible_text_entry.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/flexible_text_entry/flexible_text_entry.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("flexible_text_entry foi liberado durante o teste")
