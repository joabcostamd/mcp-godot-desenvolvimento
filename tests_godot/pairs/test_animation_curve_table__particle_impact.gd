## Teste de composicao: animation_curve_table + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: animation_curve_table
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/animation_curve_table/animation_curve_table.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/animation_curve_table/animation_curve_table.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("animation_curve_table foi liberado durante o teste")
