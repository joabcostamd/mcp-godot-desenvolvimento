## Teste de composicao: random_walker + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: random_walker
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/random_walker/random_walker.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/random_walker/random_walker.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("random_walker foi liberado durante o teste")
