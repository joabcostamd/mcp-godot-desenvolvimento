## Teste de composicao: tile_pattern_stamper + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: tile_pattern_stamper
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/tile_pattern_stamper/tile_pattern_stamper.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/tile_pattern_stamper/tile_pattern_stamper.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("tile_pattern_stamper foi liberado durante o teste")
