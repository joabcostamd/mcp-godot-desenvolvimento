## Teste de composicao: crafting + farming_plot
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: crafting
## @behavior_b: farming_plot
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/farming_plot/farming_plot.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/farming_plot/farming_plot.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("farming_plot foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("planted")).override_failure_message("Sinal 'planted' nao encontrado em farming_plot")

	# Sinal 'stage_changed': verifica conectividade
	assert_bool(_behavior_b.has_signal("stage_changed")).override_failure_message("Sinal 'stage_changed' nao encontrado em farming_plot")

	# Sinal 'ready': verifica conectividade
	assert_bool(_behavior_b.has_signal("ready")).override_failure_message("Sinal 'ready' nao encontrado em farming_plot")

	# Sinal 'harvested': verifica conectividade
	assert_bool(_behavior_b.has_signal("harvested")).override_failure_message("Sinal 'harvested' nao encontrado em farming_plot")
