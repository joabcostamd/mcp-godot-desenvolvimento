## Teste de composicao: trigger_zone + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: trigger_zone
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/trigger_zone/trigger_zone.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/trigger_zone/trigger_zone.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("trigger_zone foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("zone_entered")).override_failure_message("Sinal 'zone_entered' nao encontrado em trigger_zone")

	# Sinal 'zone_exited': verifica conectividade
	assert_bool(_behavior_a.has_signal("zone_exited")).override_failure_message("Sinal 'zone_exited' nao encontrado em trigger_zone")
