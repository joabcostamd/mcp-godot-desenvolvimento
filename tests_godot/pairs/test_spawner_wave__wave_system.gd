## Teste de composicao: spawner_wave + wave_system
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: spawner_wave
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/spawner_wave/spawner_wave.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/spawner_wave/spawner_wave.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("spawner_wave foi liberado durante o teste")
	...
	assert_bool(_behavior_a.has_signal("wave_started")).override_failure_message("Sinal 'wave_started' nao encontrado em spawner_wave")

	# Sinal 'wave_cleared': verifica conectividade
	assert_bool(_behavior_a.has_signal("wave_cleared")).override_failure_message("Sinal 'wave_cleared' nao encontrado em spawner_wave")

	# Sinal 'all_waves_done': verifica conectividade
	assert_bool(_behavior_a.has_signal("all_waves_done")).override_failure_message("Sinal 'all_waves_done' nao encontrado em spawner_wave")

	# Sinal 'enemy_spawned': verifica conectividade
	assert_bool(_behavior_a.has_signal("enemy_spawned")).override_failure_message("Sinal 'enemy_spawned' nao encontrado em spawner_wave")
