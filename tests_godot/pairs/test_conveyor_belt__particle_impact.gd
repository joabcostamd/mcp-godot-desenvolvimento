## Teste de composicao: conveyor_belt + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: conveyor_belt
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/conveyor_belt/conveyor_belt.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/conveyor_belt/conveyor_belt.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("conveyor_belt foi liberado durante o teste")
