## Teste de composicao: analytics_tracker + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: analytics_tracker
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/analytics_tracker/analytics_tracker.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/analytics_tracker/analytics_tracker.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("analytics_tracker foi liberado durante o teste")
