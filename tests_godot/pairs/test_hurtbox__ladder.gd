## Teste de composicao: hurtbox + ladder
##
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/ladder/ladder.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/ladder/ladder.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("ladder foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("climbing_started")).override_failure_message("Sinal 'climbing_started' nao encontrado em ladder")

	# Sinal 'climbing_ended': verifica conectividade
	assert_bool(_behavior_b.has_signal("climbing_ended")).override_failure_message("Sinal 'climbing_ended' nao encontrado em ladder")
