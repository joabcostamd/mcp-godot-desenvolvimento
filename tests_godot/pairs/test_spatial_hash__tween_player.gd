## Teste de composicao: spatial_hash + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: spatial_hash
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/spatial_hash/spatial_hash.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/spatial_hash/spatial_hash.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("spatial_hash foi liberado durante o teste")
