## Teste de composicao: deprecation_watcher + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: deprecation_watcher
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/deprecation_watcher/deprecation_watcher.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/deprecation_watcher/deprecation_watcher.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("deprecation_watcher foi liberado durante o teste")
