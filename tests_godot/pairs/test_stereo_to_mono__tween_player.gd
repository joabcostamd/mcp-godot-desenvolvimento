## Teste de composicao: stereo_to_mono + tween_player
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: stereo_to_mono
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/stereo_to_mono/stereo_to_mono.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/stereo_to_mono/stereo_to_mono.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("stereo_to_mono foi liberado durante o teste")
