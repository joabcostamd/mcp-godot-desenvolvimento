## Teste de composicao: scene_transition + teleport
##
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/teleport/teleport.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/teleport/teleport.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("teleport foi liberado durante o teste")
	...
	# ── Verifica que teleport ainda consegue emitir sinais ──
	# Sinal 'teleported': verifica conectividade
	assert_bool(_behavior_b.has_signal("teleported")).override_failure_message("Sinal 'teleported' nao encontrado em teleport")
