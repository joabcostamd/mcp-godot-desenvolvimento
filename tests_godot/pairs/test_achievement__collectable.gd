## Teste de composicao: achievement + collectable
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: achievement
## @behavior_b: collectable
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/collectable/collectable.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/collectable/collectable.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("collectable foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("collected")).override_failure_message("Sinal 'collected' nao encontrado em collectable")

	# Sinal 'pickup_failed': verifica conectividade
	assert_bool(_behavior_b.has_signal("pickup_failed")).override_failure_message("Sinal 'pickup_failed' nao encontrado em collectable")
