## Teste de composicao: hurtbox + lerp_smooth
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: hurtbox
## @behavior_b: lerp_smooth
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/lerp_smooth/lerp_smooth.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/lerp_smooth/lerp_smooth.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("lerp_smooth foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("completed")).override_failure_message("Sinal 'completed' nao encontrado em lerp_smooth")
