## Teste de composicao: save_migration + screen_shake
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: save_migration
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/save_migration/save_migration.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/save_migration/save_migration.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("save_migration foi liberado durante o teste")
