## Teste de composicao: accordion + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: accordion
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/accordion/accordion.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/accordion/accordion.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("accordion foi liberado durante o teste")
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("particle_impact foi liberado durante o teste")
	# ── Verifica que accordion ainda consegue emitir sinais ──
	# Sinal 'section_toggled': verifica conectividade
	assert_bool(_behavior_a.has_signal("section_toggled")).override_failure_message("Sinal 'section_toggled' nao encontrado em accordion")
