## Teste de composicao: currency + input_manager
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: currency
## @behavior_b: input_manager
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/input_manager/input_manager.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/input_manager/input_manager.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("input_manager foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("device_changed")).override_failure_message("Sinal 'device_changed' nao encontrado em input_manager")

	# Sinal 'action_rebound': verifica conectividade
	assert_bool(_behavior_b.has_signal("action_rebound")).override_failure_message("Sinal 'action_rebound' nao encontrado em input_manager")
