## Teste de composicao: enemy_chase + player_topdown
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: enemy_chase
## @behavior_b: player_topdown
...
func test_coexistencia() -> void:
	...
	var res_b := load("res://behaviors/player_topdown/player_topdown.tscn")
	assert_object(res_b).is_not_null().override_failure_message("Falha ao carregar res://behaviors/player_topdown/player_topdown.tscn")
	...
	assert_bool(is_instance_valid(_behavior_b)).override_failure_message("player_topdown foi liberado durante o teste")
	...
	assert_bool(_behavior_b.has_signal("velocity_changed")).override_failure_message("Sinal 'velocity_changed' nao encontrado em player_topdown")
