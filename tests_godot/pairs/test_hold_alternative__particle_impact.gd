## Teste de composicao: hold_alternative + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: hold_alternative
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/hold_alternative/hold_alternative.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/hold_alternative/hold_alternative.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("hold_alternative foi liberado durante o teste")
