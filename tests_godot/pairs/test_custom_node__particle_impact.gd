## Teste de composicao: custom_node + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: custom_node
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/custom_node/custom_node.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/custom_node/custom_node.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("custom_node foi liberado durante o teste")
