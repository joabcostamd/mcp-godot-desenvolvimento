## Teste de composicao: crash_reporter + particle_impact
##
## Gerado automaticamente por scripts/gerar_testes_pares.py (sota_1.7).
## Verifica que os dois behaviors coexistem sem erro e seus sinais ainda emitem.
##
## @behavior_a: crash_reporter
...
func test_coexistencia() -> void:
	var res_a := load("res://behaviors/crash_reporter/crash_reporter.tscn")
	assert_object(res_a).is_not_null().override_failure_message("Falha ao carregar res://behaviors/crash_reporter/crash_reporter.tscn")
	...
	assert_bool(is_instance_valid(_behavior_a)).override_failure_message("crash_reporter foi liberado durante o teste")
