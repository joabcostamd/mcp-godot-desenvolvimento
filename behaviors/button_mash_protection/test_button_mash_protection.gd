...
func test_defaults() -> void: var o:=ButtonMashProtection.new(); o.queue_free()

func test_edge_case_disabled() -> void:
	var c := ButtonMashProtection.new()
	c.enabled = false
...
func test_edge_case_multiple_instances() -> void:
	var a := ButtonMashProtection.new()
	var b := ButtonMashProtection.new()
