...
func test_defaults() -> void: var o:=AnimationStateMachine.new(); o.queue_free()

func test_edge_case_multiple_instances() -> void:
	var a := AnimationStateMachine.new()
	var b := AnimationStateMachine.new()
...
func test_integration_add_child() -> void:
	var c := AnimationStateMachine.new()
	add_child(c)
