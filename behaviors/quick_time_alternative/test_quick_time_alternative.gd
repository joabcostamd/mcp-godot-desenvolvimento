...
func test_defaults() -> void: var o:=QuickTimeAlternative.new(); o.queue_free()

func test_edge_case_multiple_instances() -> void:
	var a := QuickTimeAlternative.new()
	var b := QuickTimeAlternative.new()
...
func test_integration_add_child() -> void:
	var c := QuickTimeAlternative.new()
	add_child(c)
